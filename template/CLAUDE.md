...
| `UMBRACO_EXPECTED_MAJOR` | `--umbraco-expected-major` | Overrides the default target major (`UMBRACO_TARGET_MAJOR` in `config/umbraco-target.ts`) for the version-mismatch check/block — set only when deliberately targeting a different Umbraco major |
