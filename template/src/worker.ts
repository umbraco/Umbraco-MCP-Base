...
// Umbraco major this server's tools target — generated by `npm run generate`
// (see `config/umbraco-target.generated.ts`). Drives the version-mismatch
// check in createPerRequestServer(); env.UMBRACO_EXPECTED_MAJOR overrides it
// per-deployment (see `types/env.ts` in @umbraco-cms/mcp-hosted).
...
const options = {
  ...
  // server's tools were generated against — the hosted-worker counterpart of
  // the stdio entry point's version check (see `index.ts`). Checked fresh on
  // every request (never a shared/global check) since a single Worker can
  // serve different users and, under site routing, entirely different
  // Umbraco instances. A mismatch is folded into that request's `instructions`
  // rather than blocking tool calls — see the comment on
