...
describe("widgets/createConfirmedToolDefinition", () => {
  ...
  describe("when called with confirmed: true", () => {
    it("runs the confirmHandler immediately — trusts the flag", async () => {
      const tool = makeTool();
    ...
    it("strips the confirmed flag before calling the handler", async () => {
      const tool = makeTool();
      confirmHandler.mockResolvedValue({});
      await tool.handler({ id: "abc", confirmed: true } as any, {} as any);
      const [args] = confirmHandler.mock.calls[0]!;
