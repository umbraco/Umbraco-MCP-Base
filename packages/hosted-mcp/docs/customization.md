...
Enable tool selection so users can choose which tool modes they want:
...
When enabled, the consent screen shows:
...
Tool selection is the **user tier** — it can only narrow what the admin tier allows. If the admin restricts modes to `[content, media]` via `UMBRACO_TOOL_MODES`, the consent screen only shows those two modes. Users cannot enable modes beyond the admin boundary.
